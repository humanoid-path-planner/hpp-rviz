      ...
      [this, msg]() {
        for (const auto& marker : msg->markers) {
          if (waypoint_properties_.count(marker.name)) {
            RCUTILS_LOG_INFO(
                "Updating existing waypoint property for marker '%s'",
                marker.name.c_str());
            continue;
          }

          RCUTILS_LOG_INFO("Adding new waypoint property for marker '%s'",
                           marker.name.c_str());
        ...
        for (const auto& erased_name : msg->erases) {
          RCUTILS_LOG_INFO("Received erase request for marker '%s'",
                           erased_name.c_str());
        ...
        for (const auto& marker : msg->poses) {
          RCUTILS_LOG_INFO("Received pose update for marker '%s'",
                           marker.name.c_str());
        ...
        RCUTILS_LOG_INFO(
            "Finished processing update message with %zu markers, %zu erases, "
            "%zu poses",
